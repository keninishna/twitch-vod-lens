"""Pull latest, re-run pipeline, check 758s result."""
import pexpect, time
...
p.expect('[$#>]', timeout=5)
p.sendline('cd ~/twitch-vod-analyzer && git pull 2>&1')
p.expect('[$#>]', timeout=15)
print('PULL:', (p.before or '')[-200:])

p.sendline('rm -rf __pycache__ src/synthesis/__pycache__ src/synthesis/schemas/__pycache__ 2>/dev/null')
p.expect('[$#>]', timeout=5)
p.sendline('cd ~/twitch-vod-analyzer && PYTHONPATH=. python3 -u src/synthesis/qwen_clip_analyzer_progressive.py --vod-id 2770929139 --skip-audio > /tmp/pipeline_fix2.log  /tmp/pipeline_fix2.log 2>&1 &')
p.expect('[$#>]', timeout=5)
p.sendline('echo "PID=\\$!"')
p.expect('[$#>]', timeout=5)
