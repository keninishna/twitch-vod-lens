...
GEMMA_URL = os.environ.get("GEMMA_URL", "http://localhost:8084")
...
- VARY PATTERNS across clips: if one clip uses a '[quote]' or short+punchy structure, use a different pattern (reaction-based, question bait, surprise reveal) for the next. Multiple clips with the same title structure feel repetitive even when text differs.
...
- DEDUP RULE: Each clip has a unique clip_id (e.g. 'Clip at 998s'). NEVER assign the same clip_point/title to two different clips. If two clips have similar content, differentiate their titles. When in doubt, reference the clip_id as an anchor. Also VARY the title STRUCTURE across clips — don't reuse the same pattern (e.g. '[quote]' format) on multiple clips; alternate between reaction-based, question bait, short+punchy, and surprise reveal.
- TITLE RULE: Each clip's clip_point MUST be click-worthy and curiosity-inducing. Do NOT use dry descriptions. For chat-read clips, preserve attribution but use hooky phrasing (e.g. 'What happens when chat drops a message about ...?'). Avoid bland forms like 'Streamer reads a chat message about ...'. VARY title patterns across clips — don't let more than one clip share the same structure.
...
- DEDUP RULE: Each clip has a unique clip_id (e.g. 'Clip at 998s'). NEVER assign the same clip_point/title to two different clips. Each clip MUST have a unique title. Differentiate similar clips by focusing on what makes each moment distinct. Also VARY the title STRUCTURE — don't reuse the same pattern (e.g. '[quote] Stops Her Cold') on more than one clip; alternate across the 5 patterns.
- TITLE RULE: clip_point must be click-worthy. Use ONE of: reaction-based ('[Streamer] [reaction] after [trigger]'), question bait ('What happens when...?'), short+punchy ('She had ONE job'), or surprise reveal ('Girl [verb] [something] and [surprise]'). NO dry forms like 'Streamer reads a chat message about ...'. For chat-read clips, keep attribution while still hooky (e.g. 'What happens when chat drops a message about ...?'). Avoid duplicate words, repeated phrase structures, and VARY patterns across clips — don't let more than one clip use the same title structure.
